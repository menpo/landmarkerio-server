...
def load_model(name, source_dir):
    ...
    fitter = LucasKanadeAAMFitter(model, n_shape=[3, 6, 12], n_appearance=0.9)
    detector = load_dlib_frontal_face_detector()
    return model, fitter, detector
...
class Fitter(object):
    ...
    def __init__(self, name, directory):
        ...
        aam, fitter, detector = load_model(name, directory)
        self.model = aam
        self.fitter = fitter
    ...
    def fit(self, encoded_img, landmarks=None):
        ...
        if landmarks is not None:
            ...
            print('No landmarks provided - using detector')
            self.detector(img)
            ...
            init_shape = AlignmentSimilarity(ref_shape.bounding_box(),
                                             bbox.lms).apply(ref_shape)
